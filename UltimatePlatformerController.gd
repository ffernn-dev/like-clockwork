...
##Animations must be named "death" all lowercase as the check box says
...
func _physics_process(delta):
	...
	runHold = false #Input.is_action_pressed("run")
	latchHold = false #Input.is_action_pressed("latch")
	dashTap = Input.is_action_just_pressed("dash") and not dead
	rollTap = false #Input.is_action_just_pressed("roll")
